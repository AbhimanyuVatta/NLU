...
data_path_restore = '/media/abhimanyu/ADI/MTech/CREDIT_SUBJECT_SECONDSEM/NLU/Assignment_2_15251/word_logs/'
...
def parse_text(text):
    ...
    tokens_cnt = collections.Counter(tokens)
    for i in range(tokens_len):
        if tokens_cnt[tokens[i]] < 2:
            tokens[i] = 'unk'    
    return tokens
...
def train(train_data, vocabulary, num_layers, num_epochs, batch_size, model_save_name, model_path,
          learning_rate=1.0, max_lr_epoch=10, lr_decay=0.93, print_iter=100):
...
def test(model_path, test_data, reversed_dictionary):
    ...
    with tf.Session() as sess:
        ...
        print(model_path)
        #saver.restore(sess, model_path  + "lstm-final")
        check_point = tf.train.get_checkpoint_state(model_path)
        if check_point and check_point.model_checkpoint_path:
           # print("i am here")
            saver.restore(sess, check_point.model_checkpoint_path)
...
if options.task == 1:
    train(train_data, vocabulary, num_layers=2, num_epochs=num_epochs, batch_size=batch_size, model_save_name='lstm_word', model_path = data_path_restore)
